...
- 私钥：主控私钥 2/2，签名私钥与恢复私钥在开发环境中暂不配置。
<Note>两个私钥分片均由 Cobo 默认配置，因此您在开发环境中使用该 MPC 钱包发起交易时，Cobo 会自动完成交易的签名，您无需手动签名。</Note>
...
- [设置主控私钥](/cn/portal/mpc-wallets/ocw/holder-group-main-group)
- [设置签名私钥](/cn/portal/mpc-wallets/ocw/holder-group-signing-group)
- [设置恢复私钥](/cn/portal/mpc-wallets/ocw/holder-group-recovery-group)
- [主控私钥 / 签名私钥 / 恢复私钥的区别](/cn/portal/mpc-wallets/ocw/holder-group-overview)

## 设置 Fee Station（可选）
如果您希望在开发环境中使用 Cobo 的美元稳定币代付链上交易手续费功能，或集中管理主链币以支付 Gas，可以启用 Fee Station。它相当于一个团队共享的费用账户，可根据配置策略为链上交易自动代付 Gas，并统一结算各类平台费用。详情请参阅[Fee Station 简介](/cn/portal/fee-station/introduction)。

## 开启代币自动归集（可选）
如果需要开启 MPC 钱包资金的自动归集，请参考[启用自动代币归集](/cn/portal/enable-auto-sweep)。
...
为方便测试，在开发环境中，系统已为您自动分配测试币 SETH。如果需要充值其他币种（例如 USDC 或 USDT），请确认所用链（Tron / BEP-20 / ERC-20 等），并参考[充值](/cn/portal/mpc-wallets/ocw/transfers)文档。
